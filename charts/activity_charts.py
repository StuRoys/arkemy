...
def get_activity_tag_columns(df: pd.DataFrame) -> list:
    """
    Get only activity_tag* and record_tag* columns from all tag columns.

    Args:
        df: DataFrame to search

    Returns:
        List of activity/record tag columns only
        Example: ['activity_tag', 'activity_tag_1', 'record_tag_1', 'record_tag_2']
    """
    all_tags = get_all_tag_columns(df)
    return [col for col in all_tags if col.startswith('activity_tag') or col.startswith('record_tag')]

def get_widget_key(base_key: str, nav_context: str = "activities") -> str:
    """Generate stable widget keys for activity charts"""
...
def render_activity_tab(filtered_df, aggregate_by_activity, render_chart, get_category_colors):
    ...
    Args:
        ...
        aggregate_by_activity: Function to aggregate data by activity (deprecated, kept for compatibility)
    ...
    # Get available activity/record tags with labels
    ...
    # Aggregate by selected activity/record tag - this will be used for all visualizations
    activity_agg = aggregate_by_project_tag(filtered_df, tag_column=selected_tag_column)
    ...
    if activity_agg.empty:
        st.error("No activity/record data available after filtering.")
        return
    ...
    if not filtered_activity_agg.empty:
        if visualization_type == "Treemap":
            ...
            # Add all activity/record tags as children of root
            root_total = 0
            for _, row in filtered_activity_agg.iterrows():
            ...
            if pct_column in filtered_activity_agg.columns:
                for i, pct_val in enumerate(filtered_activity_agg[pct_column], 1):  # Start from 1 to skip root
            ...
            pct_column = f'{metric_column}_pct'
            has_percentage = pct_column in filtered_activity_agg.columns
            ...
            # Add a slider to control number of activities to display (only if 10+ items)
            if len(activity_agg) >= 10:
    ...
    with st.expander("Details"):
        ...
        display_df = sorted_activity_agg.copy()
