...
def create_treemap(data_df, path_col, values_col, color_scheme="Blues", unit_text=""):
    ...
    if use_thousand_sep:
        ...
        hover_template = f"<b style='font-size:18px'>%{{label}}</b><br><span style='font-size:18px'>{values_col}: %{{value:,d}} {unit_text}</span>"
        hover_template = hover_template.replace(",", " ")
        text_template = '%{label}<br>%{value:,d} ' + unit_text
